...
var S7CommConfigSpec = service.NewConfigSpec().
	...
	Field(service.NewStringField("tcpDevice").Description("IP address of the S7 PLC.").Examples("{{ .IP }}", "192.168.1.100:102", "10.0.0.50:102", "plc.local:102")).
	...
	Field(service.NewIntField("batchMaxSize").Description("Maximum count of addresses to be bundled in one batch-request (PDU size).").Default(480).Examples(480, 100, 960)).
	Field(service.NewIntField("timeout").Description("The timeout duration in seconds for connection attempts and read requests.").Default(10).Examples(10, 5, 30)).
	...
	Field(service.NewStringListField("addresses").Description("List of S7 addresses to read in the format '<area>.<type><address>[.extra]', e.g., 'DB5.X3.2', 'DB5.B3', or 'DB5.C3'. " +
		"Address formats include direct area access (e.g., DB1 for data block one) and data types (e.g., X for bit, B for byte).").Examples([]string{"DB1.DW20", "DB1.S30.10"}, []string{"DB1.DBW0"}, []string{"DB1.DBW0", "DB3.DBX270.5"}, []string{"DB5.DBB3", "DB10.DBD20"}))
