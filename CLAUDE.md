...
This file provides guidance for developing benthos-umh plugins and understanding benthos-specific patterns.
...
This file focuses on **benthos-umh development**. For related guidance:

- **Team processes** (Linear/Sentry/GitHub workflows): `/Users/jeremytheocharis/Documents/git/troubleshooting/CLAUDE.md`
- **Orchestration details** (FSM, S6, template expansion): `umh-core/CLAUDE.md`
- **Frontend integration**: `ManagementConsole/CLAUDE.md`

**Scope of this file**:
- Plugin architecture and development
- Message formats and metadata flow
- Topic naming and validation
- Data flow patterns
- benthos-specific debugging (logs, configs)

**NOT in this file**:
- How umh-core orchestrates benthos processes → see umh-core/CLAUDE.md
- How FSM state machines work → see umh-core/CLAUDE.md
- How S6 supervision works → see umh-core/CLAUDE.md
...
2. umh-core Agent reads config.yaml and generates benthos config from templates
3. umh-core FSM creates S6 service directory
4. S6 launches benthos process with generated config
5. benthos process runs and sends status updates back to umh-core

**For detailed orchestration mechanics** (FSM state machines, template expansion, S6 supervision), see `umh-core/CLAUDE.md`.

### benthos Process Locations

**Config file** (generated by umh-core):
```
/data/services/benthos-{id}/data/config.yaml
```

**Logs** (S6-managed with tai64n timestamps):
```
/data/logs/benthos-{id}/current       # Active log
/data/logs/benthos-{id}/@*.s          # Rotated logs
```
...
# Search for errors
grep -i "error" /data/logs/benthos-bridge-123/current | tai64nlocal
```

### Debugging benthos Issues

**benthos won't start**:
1. Check benthos logs (config validation errors, plugin failures)
2. Verify generated config has no `{{` template markers left
3. Check S6 process state with `s6-svstat /data/services/benthos-{id}/`

**benthos crashes**:
1. Check benthos logs for panic/fatal errors
2. Look for protocol connection errors (timeout, refused, unreachable)
3. Verify input plugin configuration (addresses, credentials)

**Data not flowing**:
1. Check benthos logs for throughput (should show message processing)
2. Verify Kafka topic exists: `rpk topic list`
3. Check UNS output writes to Kafka: `rpk topic consume umh.messages --num 10`

**For FSM/S6/umh-core debugging**, see `umh-core/CLAUDE.md`.
