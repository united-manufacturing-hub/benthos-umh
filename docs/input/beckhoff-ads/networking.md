...
ADS works from inside Docker containers with default bridge networking — **no `host_network` and no
port forwarding are needed**. Requests, responses and notifications normally all flow over the
single outbound TCP connection the plugin opens to port 48898. The exception is a device that
answers only on a connection it opens back to the client, which does need inbound `48898`; see
...
- **`hostIP` should be set** to the Docker host's IP on the PLC network (e.g. `192.168.1.50`) — this is the address stored in the route, and it has to be the one the PLC sees. Left empty it auto-detects the container's bridge IP, which the PLC cannot match.
...
- `hostIP`: IP address the PLC associates with this client. In Docker with bridge networking, set to the Docker host's IP on the PLC network — mandatory on TwinCAT 2, which routes replies via this address. Auto-detected from the outbound connection if empty (only correct with `host_network` or macvlan)
...
```mermaid
flowchart LR
  P1["<b>Sent by the bridge</b><br/>IP src 100.93.137.23<br/>AMS src NetID 100.93.137.23.1.1"]
...
With `hostIP` left as the client's own address the bridge appears to start normally — the route
...
```bash
nc -l 9999                       # on any host in the PLC's subnet, then connect from the bridge host
conntrack -L | grep 48898        # on the gateway itself, shows the translation
```

Any HTTP service already running on that subnet works too — make one request from the bridge host
and read its access log.

**Or remove the translation.** With SNAT disabled the PLC reaches the client at its own address,
and `hostIP` is simply that address — which is what fixed all three PLCs above. That needs a return route for the VPN's
...
Automatic registration ties `Address` and `NetId` together — both come from `hostIP`. A manual route
...
Adding health check etc for the PLC, in example add a connection check that port-scans the
device periodically can break the connection if not using a client side AMS router.
The one thing to avoid is pointing that check at **48898**: the TwinCAT router drops the live ADS
session every time that port is scanned, which looks like a flaky network but is self-inflicted.
`443` is the usual choice, `80` if 443 is closed on your device. The port has to be open, or the
check never reports healthy — and a web server answering there only proves the device is powered,
