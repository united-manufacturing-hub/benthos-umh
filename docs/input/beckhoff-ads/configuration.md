...
input:
  ads:
    ...
    transmissionMode: "serverOnChange2"    # TC3: falls back to serverOnChange on older firmware
...
exists, which is why `targetAMS` and `runtimePort` can be left unset and discovered.
...
| `targetAMS`, `runtimePort` | unchanged, and now optional — both are discovered from the PLC |
