...
| **cycleTime** | No | `100ms` | How often the PLC checks the symbol for changes (e.g. `100ms`, `10ms`, `1s`). Lower = more responsive but more PLC CPU. Values faster than the PLC task cycle are clamped to the task cycle. Only applies when `readType` is `notification` |
