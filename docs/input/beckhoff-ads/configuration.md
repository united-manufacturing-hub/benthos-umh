...
| **hostAMS** | No | `auto` | Host AMS net ID. Usually the IP address + `.1.1`. Must match a route on the PLC. `auto` (or empty) derives it from `hostIP` when that is set, otherwise from the outbound connection's local IP |
...
| **maxDelay** | No | `100ms` | Maximum time the PLC batches notifications before sending (e.g. `100ms`, `0s`). All changes are still delivered; this controls delivery latency vs network efficiency. `0s` = send immediately. Only applies when `readType` is `notification` |
| **cycleTime** | No | `100ms` | How often the PLC checks the symbol for changes (e.g. `100ms`, `10ms`, `1s`). Lower = more responsive but more PLC CPU. Values faster than the PLC task cycle are clamped to the task cycle. A symbol's first sample arrives at its first check, so a large value also delays startup, see [First batch completeness](how-it-works.md#first-batch-completeness). Only applies when `readType` is `notification` |
