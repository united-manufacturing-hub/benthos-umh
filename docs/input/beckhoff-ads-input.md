...
3. **Automatic route registration (UDP)**: Use the `username` and `password` config fields to have the plugin automatically register a route on the PLC before connecting. See the [Route Registration](#route-registration) section below. Using this without specifying `hostIP` and `hostAMS` creates a route using the container's internal IP. On TwinCAT 3 this works, since the NetID is only an identifier for an authorized route. On **TwinCAT 2 it does not** — see [TwinCAT 2 vs TwinCAT 3](#twincat-2-vs-twincat-3) below. Setting `hostIP` explicitly is recommended in all cases: internal container IPs are ephemeral and make it unclear where the route originates in the PLC's route list.

### Docker and Kubernetes

ADS works from inside Docker containers with default bridge networking — **no `host_network`, no port forwarding, and no open ports are needed**. All ADS traffic (requests, responses, and notifications) flows over a single outbound TCP connection to port 48898. On TwinCAT 3 the PLC never initiates connections back to the client; it sends all responses and notifications on the same TCP socket the client opened.

#### TwinCAT 2 vs TwinCAT 3

An AMS router uses a route in two ways: it accepts an incoming connection when the **source IP** matches a route's stored address, and it delivers replies either over the connection the client already opened or, failing that, by dialling the address stored in that route.

The two runtimes differ in whether they reuse the client's connection:

| | Reply delivery | Container IP without `hostIP` |
|---|---|---|
| **TwinCAT 3** | over the connection the client opened | Works — the stored address is never dialled |
| **TwinCAT 2** | by dialling the route's stored address | **Fails** — the route registers, then every read times out |

On TwinCAT 2 an unreachable stored address (a container's bridge IP behind NAT) produces a session that connects and registers successfully but never returns data — the symptom is repeated request timeouts rather than a connection error. **Behind NAT on TwinCAT 2, `hostIP` must be set** to an address the PLC can reach.
...
- **`hostIP` should be set** to the Docker host's IP on the PLC network (e.g. `192.168.1.50`) — required on TwinCAT 2, recommended on TwinCAT 3. This tells the PLC which IP address to associate with the route. If left empty, it auto-detects the container's bridge IP, which makes route entries on the PLC hard to identify.
