...
- Sparkplug B input: `secondary_active` and `primary` roles now publish `NCMD/Node Control/Rebirth` when DATA arrives with aliases that aren't in the in-memory cache — typically after a bridge restart where the broker has no retained `NBIRTH`/`DBIRTH`. Previously the cache stayed empty and tags surfaced in the Topic Browser as `…/_historian/alias_<n>` until something external (e.g. an upstream rebirth from an HMI or commissioning tool) triggered recovery. This recovery is unconditional for non-passive roles and is *not* gated by `request_birth_on_connect` (which only affects proactive discovery rebirths on new-node observation). Throttled by `birth_request_throttle` (default 1s) to prevent storms; suppressed for `secondary_passive` like the existing rebirth paths. New metric `alias_rebirths` counts triggered recoveries; new metric `alias_resolution_failures` counts per-metric cache misses (previously folded into the generic `messages_errored`).
