...
- Sparkplug B output (`role: edge_node`): NBIRTH, DBIRTH, NDEATH, DDEATH, and the LWT registration are now published with `retain=false`, matching Sparkplug v3.0 §5.4. Late subscribers no longer receive retained BIRTHs from the broker; they must request one via `NCMD/Node Control/Rebirth` or rely on a Sparkplug-Aware broker's `$sparkplug/certificates/#` topics.
