...
// TopicThreshold defines a threshold for specific topics or patterns
type TopicThreshold struct {
	Pattern   string  `json:"pattern,omitempty" yaml:"pattern,omitempty"` // Wildcard pattern like "*.temperature"
	Topic     string  `json:"topic,omitempty" yaml:"topic,omitempty"`     // Exact topic match
	Threshold float64 `json:"threshold" yaml:"threshold"`                 // Threshold value for this pattern/topic
}
...
- swinging_door: Dynamic compression maintaining trend fidelity using Swinging Door Trending (SDT)`).
		...
		Field(service.NewDurationField("max_interval").
			Description("Maximum time interval before forcing output regardless of threshold. Only applies to deadband algorithm. Prevents data from appearing 'stale' during slow changes.").
			Optional()).
		Field(service.NewObjectListField("topic_thresholds",
			service.NewStringField("pattern").
				Description("Wildcard pattern to match topics (e.g., '*.temperature'). Mutually exclusive with 'topic'.").
				Optional(),
	...
	err := service.RegisterBatchProcessor(
		...
		func(conf *service.ParsedConfig, mgr *service.Resources) (service.BatchProcessor, error) {
			...
			threshold, err := conf.FieldFloat("threshold")
			if err != nil {
				return nil, err
			}
			...
			var topicThresholds []TopicThreshold
			if conf.Contains("topic_thresholds") {
				topicThresholdConfs, err := conf.FieldObjectList("topic_thresholds")
				if err != nil {
					return nil, err
				}

				for i, topicThresholdConf := range topicThresholdConfs {
					var topicThreshold TopicThreshold

					// Get threshold (required)
					topicThreshold.Threshold, err = topicThresholdConf.FieldFloat("threshold")
					if err != nil {
						return nil, fmt.Errorf("topic_thresholds[%d]: %w", i, err)
					}

					// Get pattern or topic (mutually exclusive)
					hasPattern := topicThresholdConf.Contains("pattern")
					hasTopic := topicThresholdConf.Contains("topic")

					if hasPattern && hasTopic {
						return nil, fmt.Errorf("topic_thresholds[%d]: 'pattern' and 'topic' are mutually exclusive", i)
					}
					if !hasPattern && !hasTopic {
						return nil, fmt.Errorf("topic_thresholds[%d]: either 'pattern' or 'topic' must be specified", i)
					}

					if hasPattern {
						topicThreshold.Pattern, err = topicThresholdConf.FieldString("pattern")
						if err != nil {
							return nil, fmt.Errorf("topic_thresholds[%d]: %w", i, err)
						}
					}
					...
					if hasTopic {
						topicThreshold.Topic, err = topicThresholdConf.FieldString("topic")
						if err != nil {
							return nil, fmt.Errorf("topic_thresholds[%d]: %w", i, err)
						}
					}

					topicThresholds = append(topicThresholds, topicThreshold)
				}
			}
