...
func (mp *MessageProcessor) processWithState(msg *service.Message, dataMap map[string]interface{}, value interface{}, timestamp time.Time, seriesID string, state *SeriesState) (*service.Message, error) {
	...
	if !state.lastProcessedTime.IsZero() && timestamp.Equal(state.lastProcessedTime) {
		mp.processor.logger.Warnf("Dropping message with duplicate timestamp %v for series '%s' - subsequent messages with identical timestamps are not allowed. To prevent this issue for batched messages you can set 'msg.meta.timestamp_ms' to 'msg.meta.opcua_server_timestamp' or any other source.",
			timestamp, seriesID)
