...
// to whatever *service.Resources is handed the constructor — and uns_beta
...
// (uns_beta_integration_test.go:1146) but drives uns_beta. If this fails,
...
`, addr, group))).To(Succeed())
		...
		Eventually(func() bool {
			...
			Should(BeTrue(), "the inner redpanda input's redpanda_lag gauge never reached the outer stream's metrics registry under uns_beta's Go-side construction — the forwarder is not threading the outer res into NewRedpandaInput")
	})
})
