...
// uns_beta is a Go-registered input that consumes a single Kafka topic
...
// constructor newUnsBeta directly — uns_beta has no template
...
var _ = Describe("uns_beta config validation", Label("uns_beta"), func() {
	...
	parseAndConstruct := func(body string) (service.BatchInput, error) {
		parsed, err := unsBetaConfigSpec().ParseYAML(body, nil)
		if err != nil {
			return nil, err
		}
		return newUnsBeta(parsed, nil)
	}

	DescribeTable("newUnsBeta rejects invalid config",
	...
	It("returns a not-implemented error from ReadBatch so a valid config is distinguishable from a validation rejection", func() {
		_, _, err := (&unsBetaInput{}).ReadBatch(context.Background())
		Expect(err).To(MatchError(ContainSubstring("uns_beta: not implemented")))
	})
})
