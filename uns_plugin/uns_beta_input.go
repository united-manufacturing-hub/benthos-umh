...
// uns_beta is a Go-registered input that consumes a single Kafka
...
// Go reader), uns_beta does both in Go: it translates the lean 4-field
...
// unsBetaConfigSpec returns the ConfigSpec for the "uns_beta" input.
...
func init() {
	if err := service.RegisterBatchInput("uns_beta", unsBetaConfigSpec(), newUnsBeta); err != nil {
		panic(err)
	}
}

// newUnsBeta constructs the "uns_beta" input, rejecting invalid
...
func newUnsBeta(conf *service.ParsedConfig, res *service.Resources) (service.BatchInput, error) {
...
// unsBetaInput is the BatchInput returned by a successful build. The
// inner redpanda reader is constructed lazily on Connect (so construction
// itself never dials a broker). A zero-value unsBetaInput (inner == nil)
...
type unsBetaInput struct {
	...
	// inner is built on first Connect by buildUnsBetaInner (forwarder
...
func (i *unsBetaInput) Connect(ctx context.Context) error {
	if i.inner == nil {
		inner, err := buildUnsBetaInner(i.res, i.seedBrokers, i.kafkaTopic, i.consumerGroup, i.umhTopics)
		if err != nil {
			return err
		}
		i.inner = inner
...
func (i *unsBetaInput) ReadBatch(ctx context.Context) (service.MessageBatch, service.AckFunc, error) {
	if i.inner == nil {
		return nil, nil, errors.New("uns_beta: not implemented")
	}
	for {
...
// pattern and are rejected separately in newUnsBeta.
