  ...
  notes: "Test Notes"`
	...
	Describe("AddConnection", func() {
		It("should add a new connection", func() {
			...
			Expect(mgmtconfig.AddConnection(newConnection, cache, clientset)).To(Succeed())

			// wait for the configmap to be updated
			time.Sleep(200 * time.Millisecond)

			configMap, err := mgmtconfig.GetMgmtConfig(cache)
			...
			Expect(mgmtconfig.GetConnectionsFieldFromMgmtConfig(configMap)).To(ContainElement(MatchFields(IgnoreExtras, Fields{
		...
		It("should fail to add a connection with the same UUID", func() {
			...
			Expect(mgmtconfig.AddConnection(newConnection, cache, clientset)).To(MatchError("connection with UUID 6ba7b811-9dad-11d1-80b4-00c04fd430c8 already exists"))
		})
	})
	...
	Describe("UpdateConnection", func() {
		It("should update an existing connection", func() {
			...
			Expect(mgmtconfig.UpdateConnection(newConnection, cache, clientset)).To(Succeed())
		...
		It("should fail to update a connection with a non-existing UUID", func() {
			...
			Expect(mgmtconfig.UpdateConnection(newConnection, cache, clientset)).To(MatchError("connection with UUID " + uuid.String() + " does not exist"))
		})
	})
	...
	Describe("DeleteConnection", func() {
		It("should delete an existing connection", func() {
			...
			Expect(mgmtconfig.DeleteConnection(uuidlib.MustParse("6ba7b811-9dad-11d1-80b4-00c04fd430c8"), cache, clientset)).To(Succeed())
			...
			Expect(mgmtconfig.GetConnectionsFieldFromMgmtConfig(configMap)).ToNot(ContainElement(connectionToDelete))
		})

		It("should fail to delete a connection with a non-existing UUID", func() {
			uuid := uuidlib.New()
			Expect(mgmtconfig.DeleteConnection(uuid, cache, clientset)).To(MatchError("connection with UUID " + uuid.String() + " does not exist"))
		})
	})
})
