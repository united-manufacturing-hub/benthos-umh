...
Developer documentation for the Beckhoff ADS Benthos input plugin. For user documentation, see [docs/input/beckhoff-ads-input.md](../docs/input/beckhoff-ads-input.md).

## Overview

This plugin reads data from Beckhoff PLCs using the ADS (Automation Device Specification) protocol, built on top of the [`RuneRoven/go-ads/v2`](https://github.com/RuneRoven/go-ads) library.
...
```text
beckhoff_ads_plugin/
├── ads.go                              # Plugin surface: config, init, Connect, ReadBatch, Close
├── client.go                           # Client seam: interface + DTOs
├── client_goads.go                     # go-ads adapter (the only file wiring the ADS library)
├── connect.go                          # Connect orchestration: session, symbols, notifications
├── read.go                             # Notification + interval reads, message building
├── parse.go                            # Value typing (number/bool/string → payload + tag_type)
├── config.go                           # Config spec, validation, symbol-list parsing
├── logging.go                          # go-ads slog → benthos logger bridge
├── constants.go                        # Timing/buffer constants
...
TEST_ADS_TC3_HOST_AMS="192.168.1.50.1.1" \
make test-ads
```
...
TEST_ADS_TC2_ROUTE_USER="Administrator" \
TEST_ADS_TC2_ROUTE_PASS="1" \
TEST_ADS_TC2_HOST_AMS="192.168.1.50.1.1" \
make test-ads
```

Tests are skipped automatically when the corresponding environment variables are not set.

**Note on concurrent ADS clients:** TwinCAT allows only one TCP connection per source IP to the PLC. If another ADS client from the same machine is already connected, tests may fail with timeouts — close the other client before running. go-ads uses a random AMS source port per session, so notification handles are distinct even with the same NetID. `TEST_ADS_TC3_HOST_AMS` does not help with TCP slot contention; it is only relevant in advanced multi-session setups using different source IPs.
...
- [`RuneRoven/go-ads/v2`](https://github.com/RuneRoven/go-ads) - ADS protocol library (direct TCP, no TwinCAT router required)
