...
var adsConf = service.NewConfigSpec().
	...
	Field(service.NewStringField("hostAMS").Description("Host AMS net ID sent in ADS requests. 'auto' (or empty) derives it from hostIP when that is set, otherwise from the outbound TCP source IP.").Default("auto").Advanced().Examples("auto", "192.168.1.50.1.1")).
	...
	Field(service.NewStringField("hostIP").Description("IP address the PLC uses to reach this client; it must be the address the PLC sees, which behind NAT is not this client's own. 'auto' (or empty) detects it at connect time. Required in Docker bridge networking. When hostAMS is auto, the Host AMS NetID is derived as hostIP+.1.1.").Default("auto").Advanced().Examples("auto", "192.168.1.50")).
	...
	Field(service.NewStringEnumField("transmissionMode", "serverOnChange", "serverCycle", "serverOnChange2", "serverCycle2").Description("Notification transmission mode (notification readType only). serverOnChange2/serverCycle2 bind the check to the task owning the variable; they fall back to serverOnChange/serverCycle for any symbol whose ContextMask is 0.").Default("serverOnChange").Advanced().Examples("serverOnChange", "serverCycle", "serverOnChange2", "serverCycle2")).
