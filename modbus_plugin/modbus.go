...
var ModbusConfigSpec = service.NewConfigSpec().
	...
	Field(service.NewIntListField("slaveIDs").Description("Slave IDs to poll. Benthos polls each slave sequentially in the order specified, reads all configured addresses from each slave, and merges the results into a single message batch. All slaves share the same TCP connection. If a slave encounters an error, Benthos logs the error and continues to the next slave (except for fatal connection errors like broken pipe, which trigger reconnection).").Examples([]int{1}, []int{1, 2, 3}, []int{10, 20})).
	...
	Field(service.NewObjectListField("addresses",
		...
		service.NewIntField("address").Description("Address of the register to query").Examples(40001, 30100, 10),
		service.NewStringField("type").Description("Data type of the field").Examples("FLOAT32", "UINT16", "INT32"),
		service.NewIntField("length").Description("Number of registers, only valid for STRING type").Default(0).Examples(0, 10, 20),
